...
enum {
    ...
    OPEN,READ,WRIT,CLOS,PRTF,MALC,FREE,MSET,MCMP,MCPY,MMAP,DSYM,BSCH,STRT,DLOP,DIV,MOD,STRL,SNPR,EXIT,
    ...
    while (E++ < e_) {
        ...
        int k = snprintf(buf, len, "%%%d = %4.4s", E,
           & "LEA  IMM  JMP  JSR  BZ   BNZ  ENT  ADJ  LEV  "
            ...
            "MSET MCMP MCPY MMAP "
            "DSYM BSCH STRT DLOP DIV  MOD  STRL SNPR EXIT CLCA" [*E * 5]);
        write(lli_fd, buf, k);
        if (*E <= ADJ) {
            memset(buf, '\0', 20);
            k = snprintf(buf, 20, " %d\n", *++E);
                ...
                while (le < e) {
                    printf("%8.4s",
                           & "LEA  IMM  JMP  JSR  BZ   BNZ  ENT  ADJ  LEV  "
                             ...
                             "OPEN READ WRIT CLOS PRTF MALC FREE "
                             "MSET MCMP MCPY MMAP "
                             "DSYM BSCH STRT DLOP DIV  MOD  STRL SNPR EXIT CLCA" [*++le * 5]);
                    if (*le <= ADJ) printf(" %d\n", *++le); else printf("\n");
                }
            }
            ++line;
...
int main(int argc, char **argv)
{
    ...
    p = "break case char default else enum if int return "
        ...
        "dlopen __aeabi_idiv __aeabi_idivmod strlen snprintf exit __clear_cache void main";

    // name vector to system call
    // must match the sequence of supported calls
    scnames = malloc(21 * sizeof(char *));
